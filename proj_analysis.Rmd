...
gas_ts <- ts(gas_full$value, start = c(2001, 1), frequency = 12)
...
coal_ts <- ts(coal_full$consumption, start = c(2001, 1), frequency = 4)
...
petroleum_sarima <- sarima(petroleum_train, p = 5, d = 1, q = 1, P = 3, D = 1, Q = 1, S = 12)
petroleum_sarima

checkresiduals(petroleum_sarima$fit$residuals)
```
```{r petro_sarima_forecast, echo=TRUE}
petroleum_forecast <- sarima.for(xdata = petroleum_train, n.ahead = 20, p = 5, d = 1, q = 1, P = 3, D = 1, Q = 1, S = 12, plot = TRUE)
```
...
tbats_model <- tbats(petroleum_train)
...
petroleum_prophet <- prophet(petroleum_prophet_df)
...
petro_df_prophet <- head(petroleum_forecast, 230)
...
coal_sarima <- sarima(coal_train, p = 5, d = 1, q = 1, P = 3, D = 1, Q = 1, S = 12)
coal_sarima

checkresiduals(coal_sarima$fit$residuals)
```
```{r coal_sarima_forecast, echo=TRUE}
coal_forecast <- sarima.for(xdata = coal_train, n.ahead = 20, p = 5, d = 1, q = 1, P = 3, D = 1, Q = 1, S = 12, plot = TRUE)
```
...
future <- make_future_dataframe(coal_prophet, periods = 20, freq = "month")
coal_forecast <- predict(coal_prophet, future)
plot(coal_prophet, coal_forecast)
```

```{r coal_prophet_plot, echo=TRUE}
coal_df_prophet <- head(coal_forecast, 230)
