...
# plot the manual sarima forecast v.s. actual
autoplot(petroleum_forecast$pred, series = "Forecast") +
  autolayer(petroleum_test, series = "Actual") +
  xlab("Year") + ylab("Petroleum Consumption") +
...
```{r petro_sarima_plot, echo=TRUE}
sarima_result <- sarima.for(xdata = ts_data, n.ahead = 20, p = 12, d = 1, q = 2, P = 1, D = 1, Q = 1, S = 12)
...
ggplot() +
  geom_line(data = data_df, aes(x = time, y = value), color = 'black', size = 0.5) +
  geom_line(data = pred_df, aes(x = time, y = pred), color = 'red', size = 0.5) +
  geom_ribbon(data = pred_df, aes(x = time, ymin = lower, ymax = upper), alpha = 0.2, fill = 'red') +
  labs(title = 'Petroleum Forecasts with SARIMA((12, 1, 2)(1, 1, 1)[12]) Predictions', x = 'Time', y = 'Value', size=2) + theme_tufte() + 
...
```{r petro_test_error, echo=TRUE}
# get test error between post-2020 data and the forecasted values
petro_forecast <- sarima.for(xdata = ts_data, n.ahead = 25, p = 12, d = 1, q = 2, P = 1, D = 1, Q = 1, S = 12)

petro_test_rmse <- calc_rmse(petroleum_test, petro_forecast$pred)

cat("RMSE for petroleum forecast:", petro_test_rmse)

```

## Fitting natural gas model
...
```{r gas_test_error, echo=TRUE}
# get test error between post-2020 data and the forecasted values
ts_data <- gas_train

gas_forecast <- sarima.for(xdata = ts_data, n.ahead = 49, p = 7, d = 2, q = 1, P = 2, D = 1, Q = 1, S = 12)
gas_test_error <- calc_rmse(gas_test, gas_forecast$pred)

cat("Test RMSE:", gas_test_error)
...
```{r coal_test_error, echo=TRUE}}

coal_forecast <- sarima_forecast$mean
coal_test_rmse <- sqrt(mean((sarima_forecast$mean - coal_test)^2))

cat("Test RMSE:", coal_test_rmse)
```

# Conclusion

```{r combine_tests, echo=TRUE}
library(gridExtra)

# Combine RMSE test results coal_test_rmse, gas_test_error, petro_test_rmse

test_errors <- c(coal_test_rmse, gas_test_error, petro_test_rmse)

# Create a data frame with the test errors
test_errors_df <- data.frame(fuel = c("Coal", "Gas", "Petroleum"), rmse = test_errors)

# Plot the test errors
ggplot(test_errors_df, aes(x = fuel, y = rmse)) +
  geom_bar(stat = "identity") +
  labs(title = "Test RMSE for Different Fuels",
       x = "Fuel",
       y = "RMSE")
